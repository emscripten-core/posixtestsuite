 ...
 * Author: HongJunxin
 * 
 * Test: Semaphore schedule policy based on waiting time
 * tasks with same priority
 *
 * Test step:
 * 1. Main initilizes semaphore with value 0
 * 2. Main create thread A with sched_get_priority_min(). A execute sem_wait.
 * 3. Main create thread B with sched_get_priority_min(). B execute sem_wait.
 * 4. Main create thread C with sched_get_priority_min(). C execute sem_wait.
 * 5. Main execute sem_post.
 * 6. Thread A go ahead from sem_wait and then do sem_post.
 * 7. Thread B go ahead from sem_wait and then do sem_post.
 * 8. Thread C go ahead from sem_wait and then do sem_post.
 *
 */
...
static void *fn_a(void *arg)
{
	...
	if (thread_b_status == THREAD_EXIT || 
		...
		printf("Test FAILED. Thread B and C should run done after A\n");
		exit(PTS_FAIL);
	}
	...
	printf("[Thread A] sem_post and exit\n");
}
...
static void *fn_b(void *arg)
{
	...
	if (thread_a_status != THREAD_EXIT || 
			thread_c_status != THREAD_ENTER) {
		printf("Test FAILED. Thread A should run done and thread C should not exit\n");
...
static void *fn_c(void *arg)
{
	...
	if (thread_b_status != THREAD_EXIT || 
			thread_a_status != THREAD_EXIT) {
		printf("Test FAILED. Thread A and B should run done before C\n");
		exit(PTS_FAIL);
	}
